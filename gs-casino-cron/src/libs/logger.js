...
class Logger {
  ...
  static #generateWrapStr(logTitle, wrapWord) {
    return `${wrapWord}${"=".repeat(15)}${logTitle.toUpperCase()}${"=".repeat(
      15
    )}${wrapWord}`;
  }
  ...
  static #buildMessage(logAttrs) {
    ...
    if (logAttrs.klass) {
      msg.class = logAttrs.klass.name;
    }
    if (logAttrs.message) {
      msg.message = logAttrs.message;
    }
    if (logAttrs.context) {
      msg.context = logAttrs.context;
    }
    if (logAttrs.metadata) {
      msg.metadata = logAttrs.metadata;
    }
    if (logAttrs.exception) {
      msg.exceptionBacktrace = logAttrs.exception.stack;
    }
    if (logAttrs.fault) {
      msg.fault = logAttrs.fault;
    }
    return msg;
  }
}
