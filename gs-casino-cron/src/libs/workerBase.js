...
class WorkerBase {
  ...
  async #tryExecuting() {
    ...
    try {
      ...
      logger.error("Exception raised in Worker", {
  ...
  async #validateWorkerInputs() {
    ...
    if (schema) {
      ...
      if (!valid) {
        ...
        logger.debug("Worker input Validation Failed", {
          klass: this.constructor,
   ...
   */
  static async run() {
    logger.info(`Worker Started: ${this.name}`, {
      context: this.args,
      userCtx: this.context,
      wrap: "start",
    });
    ...
    logger.info(`Worker Finished: ${this.name}`, {
      context: this.args,
      userCtx: this.context,
      wrap: "end",
    });
