...
const customFormat = printf((info) => {
  ...
  msg += info.logTitle
    ? `${info.logTitle} Message: ${info.message || "No Message"} `
    : info.message || "No Message ";
  msg += info.class
    ? `class: ${
        typeof info.class === "object" ? util.inspect(info.class) : info.class
      } `
    : "";
  msg += info.context
    ? `context: ${
        typeof info.context === "object"
          ? util.inspect(info.context)
          : info.context
      } `
    : "";
  msg += info.metadata
  ...
  msg += info.exceptionBacktrace
    ? `exceptionBacktrace: ${
        typeof info.exceptionBacktrace === "object"
          ? util.inspect(info.exceptionBacktrace)
          : info.exceptionBacktrace
      } `
    : "";
  msg += info.fault
    ? `fault: ${
        typeof info.fault === "object" ? util.inspect(info.fault) : info.fault
      } `
    : "";
  return msg;
});
