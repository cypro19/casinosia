  ...
  return (req, _, next) => {
    ...
    if (compiledQuerySchema) {
      if (!compiledQuerySchema(req.query)) {
        error = true;
        errorPayload.query = ajv
          .errorsText(compiledQuerySchema.errors, { separator: " ||||| " })
          .split(" ||||| ");
      }
    }
    ...
    if (compiledParamsSchema) {
      if (!compiledParamsSchema(req.params)) {
        error = true;
        errorPayload.params = ajv
          .errorsText(compiledParamsSchema.errors, { separator: " ||||| " })
          .split(" ||||| ");
      }
    }
    ...
    if (compiledBodySchema) {
      if (!compiledBodySchema(req.body)) {
        error = true;
        errorPayload.body = ajv
          .errorsText(compiledBodySchema.errors, { separator: " ||||| " })
          .split(" ||||| ");
      }
    }
