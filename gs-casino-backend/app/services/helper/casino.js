...
export const getWagerLimitAmounts = async ({ walletId }) => {
  ...
  return {
    totalBetAmountToday:
      (await db.CasinoTransaction.sum("amount", { where: todayQuery })) +
      (await db.CasinoTransaction.sum("nonCashAmount", { where: todayQuery })),
    totalBetAmountWeekly:
      (await db.CasinoTransaction.sum("amount", { where: weekQuery })) +
      (await db.CasinoTransaction.sum("nonCashAmount", { where: weekQuery })),
    totalBetAmountMonthly:
      (await db.CasinoTransaction.sum("amount", { where: monthQuery })) +
      (await db.CasinoTransaction.sum("nonCashAmount", { where: monthQuery })),
  };
};
...
export const getLossLimitAmounts = async ({ userId }) => {
  ...
  const totalLossAmountToday = await db.CasinoTransaction.findOne({
    ...
    attributes: [
      [
        db.sequelize
          .literal(`ROUND(cast(sum(case when action_type = 'bet' then amount else 0 end) -
      sum(case when action_type = 'win' then amount else 0 end)as numeric),2) `),
        "amount",
      ],
      [
        db.sequelize
          .literal(`ROUND(cast(sum(case when action_type = 'bet' then non_cash_amount else 0 end) -
...
export const checkLimits = async ({ userId }) => {
  ...
  const { totalBetAmountToday, totalBetAmountWeekly, totalBetAmountMonthly } =
    await getWagerLimitAmounts({ walletId: wallet.walletId });

  const {
    totalLossAmountToday,
    totalLossAmountWeekly,
    totalLossAmountMonthly,
  } = await getLossLimitAmounts({ userId });
...
export const checkDepositLimit = async ({
  ...
  } = await getDepositLimitAmounts({ userId });
  ...
  if (
    ...
    totalDepositAmountToday + depositAmount > userLimits.dailyDepositLimit
  ) {
    return {
      limitReached: true,
      message: "Daily Deposit limit reached",
      limitTable,
    };
  }
  if (
    userLimits.weeklyDepositLimit &&
    totalDepositAmountWeekly + depositAmount > userLimits.weeklyDepositLimit
  ) {
    return {
      limitReached: true,
      message: "Weekly Deposit limit reached",
      limitTable,
    };
  }
  if (
    userLimits.monthlyDepositLimit &&
    totalDepositAmountMonthly + depositAmount > userLimits.monthlyDepositLimit
  ) {
    return {
      limitReached: true,
      message: "Monthly Deposit limit reached",
      limitTable,
    };
  }
...
export const getLimitTable = async (userId) => {
  const depositLimitTable = await checkDepositLimit({
    userId,
    tableOnly: true,
  });
...
export const mapFavoriteGame = (casinoGames, favoriteGame) => {
  casinoGames.forEach((game) => {
    if (game["MasterGameSubCategory.masterGameSubCategoryId"]) {
      game.masterGameSubCategoryId =
        game["MasterGameSubCategory.masterGameSubCategoryId"];
      delete game["MasterGameSubCategory.masterGameSubCategoryId"];
    }
    if (game["MasterGameSubCategory.name"]) {
      game.subCategoryName = game["MasterGameSubCategory.name"];
      delete game["MasterCasinoProvider.name"];
    }
    if (game["MasterCasinoProvider.name"]) {
      game.providerName = game["MasterCasinoProvider.name"];
      delete game["MasterGameSubCategory.name"];
    }
    if (favoriteGame)
      game.isFavorite = !!favoriteGame.find(
        (favGame) => favGame.masterCasinoGameId === game.masterCasinoGameId
      );
  });
  return casinoGames;
};
