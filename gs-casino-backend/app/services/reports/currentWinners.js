...
export class GetCurrentWinnerService extends ServiceBase {
  get constraints() {
    return constraints;
  }
  ...
  async run() {
    ...
    try {
      ...
      query = filterByDateCreatedAt(
        query,
        lastWeek.toISOString().substring(0, 10),
        today,
        "CasinoTransaction"
      );
      ...
      const currentWinners = await db.CasinoTransaction.findAll({
        ...
        attributes: [
          ...
          NULLIF(sum(case when action_type = 'bet' then amount else 0 end),0) * 100 as numeric),2)`),
            "payoutPercentage",
          ],
          [
            db.sequelize.literal(
              "ROUND(cast(sum(case when action_type = 'win' then amount else 0 end) as numeric),2)"
            ),
            "win",
          ],
        ],
